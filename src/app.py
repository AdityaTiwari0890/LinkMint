...
def check_urlscan(url, api_key):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            time.sleep(5)
            report_response = requests.get(f'https://urlscan.io/api/v1/result/{uuid}/', 
...
def check_abuseipdb(url, api_key):
    ...
    try:
        ...
        if is_ip:
            # Check IP
            url_check = f"https://api.abuseipdb.com/api/v2/check"
...
with tab3:
    ...
    **Built for production reliability with explainable AI decisions.**
    ...
    st.markdown("**🔗 GitHub:** [AdityaTiwari0890](https://github.com/AdityaTiwari0890)")
